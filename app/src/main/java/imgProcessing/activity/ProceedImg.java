...
public class ProceedImg {
    ...
    public static ArrayList<String> detectSigns(byte[] bytes, Context context, File imageFolder, TextToSpeech tts, MediaPlayer circleSound, MediaPlayer redAreaSound){
        ...
        if(Config.SAVE_BITMAP_STEPS)
            saveBitmap(baseImg,context,imageFolder, "HSV_redBinarization");
        ...
        for(Area area : areas) {
            ...
            if(Config.SAVE_BITMAP_STEPS)
                saveBitmap(newArea, context, imageFolder, "before_detect" + (index++));
            ...
            if(Config.SAVE_BITMAP_STEPS)
                saveBitmap(newArea, context, imageFolder, "areaBlack" + (index++));
           ...
           for (int i = 0; i < circles.size(); i++) {
               ...
               if(Config.SAVE_BITMAP_STEPS)
                   saveBitmap(newArea2, context, imageFolder, "outColor" + (indexR));
               ...
               if(Config.SAVE_BITMAP_STEPS)
                   saveBitmap(newArea2, context, imageFolder, "outBlackBinarization" + (indexR));
                ...
                for (Area areaDigit : digitsAreas) {
                    indexDigit++;
                    tmp += String.valueOf(checkDigit(newArea2,areaDigit,context,imageFolder,indexDigit));
                }
        ...
        if((detected.size()>0)&&(Config.SAVE_RESULT))
            saveBitmap(rgbImgOut, context, imageFolder, "result");
        return detected;
    }

    private static String checkDigit(MyImage ImgToClone, Area area,  Context context, File mImageFolder, int indexDigit){
        ...
        if(Config.SAVE_BITMAP_STEPS)
            saveBitmap(tmp,context,mImageFolder,"digit" + indexDigit);

        return  String.valueOf(PatternChecker.recognisePattern(tmp));
    }

    private static void saveBitmap(MyImage myImage, Context context, File mImageFolder, String prefix){

        ByteArrayOutputStream stream = new ByteArrayOutputStream();
        myImage.getImage().compress(Bitmap.CompressFormat.PNG, 100, stream);
        ...
        try {
            imageFile = File.createTempFile(prefix,".png",mImageFolder);
